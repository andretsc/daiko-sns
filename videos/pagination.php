...
if($page>1){
    echo "<li><a href=' " . htmlspecialchars($_SERVER['PHP_SELF']) . " ' title='Voltar para primeira.'>";
    echo " << 先頭ページ";
    echo "</a></li>";
}

// count all rows in the database
 $prep_state= $cmd->gettodos();
$total_rows = $prep_state->rowCount();
...
if($page<$total_pages){
    echo "<li><a href='" . htmlspecialchars($_SERVER['PHP_SELF']) . "?page={$total_pages}' title='Ultima {$total_pages}.'>";
    echo "最後のページ>> ";
    echo "</a></li>";
}
